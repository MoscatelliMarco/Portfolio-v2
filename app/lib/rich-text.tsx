...
export function parseMarkdownParagraphs(markdown: string): ParsedParagraph[] {
  return markdown
    .replace(/<!--[\s\S]*?-->/g, "")
    .trim()
    .split(/\n\s*\n/)
    ...
    .filter(Boolean)
    .map((paragraph, index) => parseParagraph(paragraph, index));
}

export function parseParagraph(text: string, index: number): ParsedParagraph {
  return {
    key: `${index}-${text.slice(0, 24)}`,
    children: renderInlineNodes(parseInlineNodes(text), `${index}`),
  };
}
...
function parseNodesUntil(
  ...
  while (state.cursor < text.length) {
    ...
    const isSelfClosing =
      rawTag.endsWith("/>") || tagName === "age" || tagName === "icon";
